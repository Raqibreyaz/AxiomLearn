...
const CourseCard = ({ course }: CourseCardProps) => {
  const domain =
    domainConfig[course.domain?.toUpperCase?.() ?? ""] ?? fallbackDomain;
  ...
  const hours =
    course.totalHours ??
    (lessonCount ? `${Math.ceil(lessonCount * 0.2)}h` : "");
      ...
      <div className="bg-paper border border-line rounded-md overflow-hidden cursor-pointer transition-all duration-[180ms] hover:-translate-y-1 hover:border-t3 shadow-card">
        ...
        <div className="h-[150px] relative flex items-center justify-center overflow-hidden">
          {course.thumbnail ? (
            ...
            >
              <span className="text-[30px] select-none" aria-hidden="true">
                {domain.glyph}
              </span>
            </div>
          )}
        ...
        <div className="p-4">
          ...
          {course.shortDescription && (
            <p className="text-[12.5px] text-t2 leading-[1.55] mb-[13px] line-clamp-2">
              {course.shortDescription}
            </p>
          )}
